...
| CYN summer-guild absence (bias −0.89) | P3 — **re-labelled 2026-08-23 (doc §28): the miss is SUMMER, not cool-season** | Obs CYN_C peaks **Aug 2.30 mg C/L vs model 0.06 (0.03×)**; Jun–Oct all ≤0.27×, while Feb–Mar are roughly right. Survives every configuration and both DE attractors (§25: two parameter sets, same composition outcome — the data constrain the outcome, not the route). Grazing structure is the suspect (obs ZOO now at scale, but preference/refuge untested); several published cyanobacteria models use a toxicity grazing refuge (`Diazotroph_phenology_modelling_review.md`). |
