...
- **Manuscript split + repository hygiene (2026-09-06)** — the combined calibration draft was
  split into a **two-paper pair**: *Paper I* (`docs/CL29_Model_Enhancements_Paper_Draft.md`, the six
  process mechanisms as development — formulation, bit-identical verification, adoption decision,
  plus the five built-but-disabled options that appeared in no earlier draft) and *Paper II*
  (`docs/CL29_Calibration_Paper_Draft.md`, the calibration programme and the complete nine-class
  ...
  counts (`nconst` 318→324), and the **manuscript figure generators** now tracked
  (`docs/report_scripts/make_figures*.py` + their `.csv` inputs; the build chain stays ignored).
  PRs #120, #121.
...
- **The light-climate + cyano-partition arc (2026-09-03 → 09-05, doc §44–55)** — ice cover ✅**ADOPTED** (§45, the
  study's first code+forcing change: Feb `DIA_C` 2.89×→0.99×, seasonal r +0.74). Then **four measured negatives in a row,
  every mechanism built opt-in and byte-identical when off, nothing adopted**: Form B day-length weighting (§47–49 —
  correct physics that undoes the ice fix, and a 192-eval DE could not recover the compensation); and the two-guild
  cyanobacterial coexistence (§50–55 — **nine parameter directions, over-determined**). ⭐⭐ The arc's biggest single
  re-measurement: **the Aug–Oct autumn residual is a PARTITIONING error, not a biomass deficit** — total cyanobacterial
  carbon is already right to 1–5 % and sits in the wrong guild, with September CHLA at 0.98× while all four groups are
  wrong. Also fixed en route: the `smith == 0` day-length bug (7 sites, both-regimes regression test), and two
  calibration-harness defects able to corrupt a run silently (600 s worker timeout against 541 s evaluations; the
  defaults absent from the DE's initial population). Full ledger: `docs/CL29_phenology_diagnosis.md` §44–55.
...
| **Second diatom guild (June collapse / August return)** | P2 — **re-measured again 2026-09-04 (doc §50): STANDS and is now the largest single NET autumn component, but its Aug–Sep case is QUALIFIED** | ⭐ **§50's decomposition (Sep+Oct+Nov, obs-matched, exact):** `DIA_C` **−1.435** mg C/L is the biggest *net* deficit (CYN −2.512 and FIX +1.856 largely cancel to −0.656; OPA −1.126), so the guild addresses the largest net gap — but only ~45 % of the −3.217 total. ⚠ **The qualification:** the warm-guild spec argues its case on **Aug–Sep**, and that is exactly where the model already produces the right *total* cyanobacterial carbon in the wrong guild (§50.2) — a warm diatom guild added there enters a niche the model over-fills. **§55 also shows the autumn diatom deficit is NOT downstream of the fixer excess**: across the whole `KG_NOST` sweep `DIA_C` never moves off 0.004–0.005 (obs 0.557), so settling the cyanobacterial partition would not have fixed the diatoms either. Spec `docs/superpowers/specs/2026-09-03-warm-diatom-guild-design.md` §2.2 is flagged in place. Earlier detail: | Post-boundary obs-matched monthly ratios: cool-season excess Feb 2.29× / Mar 1.90× / **Jun 1.94×** (the observed June clear-water collapse to 0.196 mg C/L is not reproduced), warm-season absence total (Jul 0.08×, **Aug 0.005×**, Sep 0.01×, Oct 0.03×). Si carries the matching signature — Aug–Oct still **2.0–2.2×** (the §26 boundary fix bought the RMSE via autumn–winter drawdown; the *summer* Si consumer is still missing). One CTMI envelope cannot do both sides — the wide-envelope experiment (doc §11) fixed Si/winter but destroyed June and ate the fixers' P. Cool guild lit T_opt≈14 (*Stephanodiscus*); the warm guild (*Actinocyclus*/*Skeletonema subsalsum*) has no clean published cardinals. |
...
| **Two-guild cyanobacterial coexistence (CYN vs the staged fixer)** | **DECIDED — OVER-DETERMINED, a bounded structural limit (2026-09-04/05, doc §50–55).** ⭐⭐ The Aug–Oct autumn residual is **not a biomass deficit but a PARTITIONING error**: total cyanobacterial carbon is already right to within 1–5 % (0.99×/0.96×/1.05×) and sits in the wrong guild (CYN 0.10–0.27×, FIX 1.71–2.27×) — invisible until now because every aggregate metric sums the two pools. **September CHLA is 0.98× with the composition entirely wrong**, which is §22's "validate on group carbon, never chlorophyll" rule with its sharpest number. ⚠⚠ **Root cause: §31's role swap moved the fixer role to a guild parameterised differently — in THREE places** — no DIN-gated fixation switch (`FIX_CYN` has `K_FIX/(K_FIX+DIN)`, NOST had a fixed 10 % share, so the guild carrying all the fixer biomass grows **80–90 % on DIN** and is not a diazotroph); `PREF_ZOO_NOST_VEG_HET` **0.00** vs `FIX_CYN`'s 0.07; and a positional ratchet whose rate constants are global. ⭐ `FIX_CYN_C` is 0.002 — the fixer pool is entirely `NOST_VEG_HET_C`, so the obviously-named knob gives a false negative (sibling of the parallel-code-paths trap). **Three opt-in mechanisms were built, all byte-identical off, none adopted:** `NOST_FIX_SWITCH` (`d9896b7`, worth **2.3–4.7× more fixer** against a matched-rate control — genuinely necessary), `R_FIX_NOST` (`47a1ea2`, a real but weak second lever), `CYANO_POS_GUILDS` (`8ccad39`, per-guild ratchet). ⭐⭐⭐ **NINE parameter directions, best (share,total) frontier lift +0.050, the gate (share 0.35–0.65 *with* total ≥0.80×) never met.** The reason is theory, not exhaustion: the guilds are measurably **identical on all four axes** — nitrogen, phosphorus, light (every `I_S_*` is inert; `I_s = GITMAX·C:Chl` and both are C:Chl 78) and vertical position — so self-limitation **equals** cross-limitation, the neutral case: winner-take-all with a knife-edge instead of an attractor. And the two requirements conflict outright: coexistence needs `KG_NOST` < `KG_CYN` = 2.0 while persistence needs 7.6 (§31's value), with the fixer **functionally extinct (0.037×)** below 2.0 even with the switch protecting it. ⭐⭐ **Twice the intuitive repair helped the WRONG guild** — grazing NOST makes NOST grow *more* (recycling + a larger ZOO pressing weak CYN), and the *ecologically correct* vertical pairing (scum-forming NOST surfaces, shade-adapted CYN mixed) moves the share the wrong way; removing CYN's grazing **entirely** moves it 0.2 %. **Where one competitor already dominates, any symmetric transfer of a resource or a stress is captured by the dominant guild.** ⇒ Remaining exits are structural, none a parameter: a resource only one guild uses (the switch grants a *refuge*, not a separate resource); density-dependent self-limitation for phytoplankton (the quadratic closure ZOO already has, §36); or a genuinely different functional type. Nothing in `INPUTS_CL29/` changed; §31's configuration stands. |
| **Day-length weighting of the light limitation (`LIGHT_DAYLENGTH_OPTION`)** | **MEASURED NEGATIVE — recorded 2026-09-04 (user decision), doc §47–49.** Built and merged (`60772ae`): `0` = legacy 24-h light (default, byte-identical at full record **and** on the 0D golden), `1` = Form A `FDAY·f(I_A)`, `2` = Form B `FDAY·f(I_A/FDAY)`. **Form B is correct physics and is NOT adopted.** `FDAY` was read, allocated and bundled but never reached the `smith == 1` path CL29 runs, while being fully wired on the `smith == 0` library branch — where **Form A is a genuine bug**: it weights by the photoperiod without concentrating the dose, discarding (1−FDAY) of each day's light (71 % in December) — ✅ **that branch bug is FIXED (2026-09-04)**, 7 sites, with a both-regimes regression test; operationally inert since `smith` is hardcoded to 1, verified full-record byte-identical. Landing the correct form costs ≈20 % of production in every month and **undoes §45's adopted ice fix**: February `DIA_C` 0.99× → **0.22×** (obs 0.280), October `PHYTO_TOT_C` 0.68× → 0.50×; CHLA RMSE 23.83 → 24.72, PO4 +34 %, while every over-predicted nutrient improves (TP −19 %, TN −4.6 %) and two of three phase metrics improve (autumn:spring 2.25 → 2.16 vs obs 2.06; Aug/Sep pair marginally the right way round — a 2 % margin, not a real peak shift). **Recalibration was then tried and failed (§49):** a 192-evaluation DE over the new `formb_closure` paramset bought Φ +4.6 % on the calibration window by raising every growth rate **and** diatom mortality +43 %, which took February to **0.15×** and moved autumn:spring *away* from observation — and the gain vanished on the full record (Si −5.5 % the only survivor; CHLA/PO4 marginally worse). ⇒ **the compensation for the 24-h-light error is not recoverable through these constants under this objective** — the sixth documented compensation channel, and the first found by making the model *more* correct. ⭐ Two results worth carrying forward: `I_s ∝ GITMAX` means **GITMAX cancels in the light-limited regime**, so no growth constant can move February (measured ×1.07 for a +23 % `KG_DIA`) — the lever is `KD_DIA_20` (×3.68 at its *lower bound*, against the ×4.52 February needs, i.e. insufficient at its extreme); and **a uniform *rate* change is not a uniform *biomass* change** — Form B's rate differential is 1.04× while its biomass differential is **4.66×**, because months near the growth/loss balance integrate a sign flip exponentially. Flag stays at `0`; nothing in `INPUTS_CL29/` changed. |
