...
- [ ] 2.1 Modularize app.py — `create_ui()` split done (§2.1 Phase 2); `server()` build-cluster (`build_commands.py`) and box-network cluster (`box_network.py`) done (§2.1 Phase 3); remaining `server()` clusters (file-I/O, plot-prep, mass-balance, observations, scenarios) / Shiny-modules still open
