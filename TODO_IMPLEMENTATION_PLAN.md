...
- [ ] 2.1 Modularize app.py — `create_ui()` split done (§2.1 Phase 2); `server()` build-cluster pilot done (§2.1 Phase 3, `build_commands.py`); remaining `server()` clusters (file-I/O, plot-prep, mass-balance, observations, scenarios) / Shiny-modules still open
