...
- [ ] 2.1 Modularize app.py — `create_ui()` split done (§2.1 Phase 2); `server()` build-cluster (`build_commands.py`), box-network cluster (`box_network.py`), and output-data cluster (`output_data.py`) done (§2.1 Phase 3); remaining `server()` clusters (mass-balance, observations, scenarios, reactive CSV cache/`run_command`/`analyze_output_directory`, inline command-logic copy, `_execute_build_process`) / Shiny-modules still open
