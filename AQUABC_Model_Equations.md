...
An interactive network diagram showing all state variables, sub-models, and their biogeochemical interactions is available:

**[Open Interactive Ecological Model Diagram](docs/aquabc_ecological_model.html)**

The diagram is generated by `tools/ecological_model_diagram.py` (requires `pyvis`). To regenerate:
...
Nodes are colour-coded by component (green = phytoplankton, red = zooplankton, blue = nutrients, purple = redox species, brown = sediment, teal = macroalgae, pink = allelopathy). Hover over any node for parameter details; edge widths reflect relative process importance.
