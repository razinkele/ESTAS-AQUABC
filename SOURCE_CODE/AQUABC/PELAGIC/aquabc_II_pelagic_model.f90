...
subroutine AQUABC_PELAGIC_KINETICS &
    ...
    character(len = 34)  , allocatable, dimension(:)   :: CO2SYS_HEAD_LOCAL
...
contains
    ...
    subroutine pelagic_derivatives(ns, ne, nkn_local)
        ! Final derivative assembly + negativity clamping + diagnostic/process-rate
        ! dumps phase (TODO 1.6 -- verbatim lift). Shared (nkn) arrays and host-scope
        ! scalars (TIME, nstate, nflags, NDIAGVAR, node_active, error, debug_stranger,
        ! CLAMP_* arrays, etc.) reached by host association. Orphaned !$omp
        ! barrier/master/critical directives bind to the enclosing parallel region
        ! at run time; none are split across the procedure boundary. The clamping
        ! scalars below are used only in this procedure and are therefore declared
        ! as locals (auto-private per OpenMP thread) rather than passed in.
        integer, intent(in) :: ns, ne, nkn_local
        integer :: i, k   ! local loop indices (auto-private per thread)
