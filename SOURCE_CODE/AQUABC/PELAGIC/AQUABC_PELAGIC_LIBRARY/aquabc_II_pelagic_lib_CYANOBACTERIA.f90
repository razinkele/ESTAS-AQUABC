...
!    end if
    ...
    if (smith .eq. 0) then
        ...
        ALPHA_0 = I_A / (max(1.0D-6, min(1.0D0, FDAY)) * I_S_CYN)
        ...
        LIM_KG_CYN_LIGHT = max(0.0D0, min(1.0D0, LIM_KG_CYN_LIGHT))
    end if
...
subroutine CYANOBACTERIA_BOUYANT &
    ...
    if (smith .eq. 1) then
        ...
        if (CYANO_POS_MODEL >= 2 .and. (CYANO_POS_GUILDS == 0 .or. CYANO_POS_GUILDS == 2)) then
            X_POS = max((EUPHOTIC_DEPTH - 0.7006D0) / 0.8121D0, W_CRIT_POS_MIN, 0.0D0)
            F_CALM = CALM_FRACTION(WINDS, X_POS)
