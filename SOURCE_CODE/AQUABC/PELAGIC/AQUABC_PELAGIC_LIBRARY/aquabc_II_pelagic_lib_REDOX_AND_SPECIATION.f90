...
subroutine REDOX_AND_SPECIATION &
    ...
    type(t_redox_lim),    intent(inout) :: redox_lim
    ...
    associate( &
        ...
        S_MINUS_2  => redox_state%S_MINUS_2,  &
        MN_II      => redox_state%MN_II,      &
        FE_II      => redox_state%FE_II,      &
        HCO3       => redox_state%HCO3,       &
        CO3        => redox_state%CO3         &
    )
    ...
    LIM_NO3N_RED = &
        (NO3N               / (NO3N + K_HS_NO3N_RED_LIM)) * &
        (K_HS_DOXY_RED_INHB / (DOXY + K_HS_DOXY_RED_INHB))

    LIM_MN_IV_RED = &
        (MN_IV              / (MN_IV + K_HS_MN_IV_RED_LIM)) * &
        (K_HS_DOXY_RED_INHB / (DOXY  + K_HS_DOXY_RED_INHB)) * &
        (K_HS_NO3N_RED_INHB / (NO3N  + K_HS_NO3N_RED_INHB))

    LIM_FE_III_RED = &
        (FE_III              / (FE_III + K_HS_FE_III_RED_LIM)) * &
        (K_HS_DOXY_RED_INHB  / (DOXY   + K_HS_DOXY_RED_INHB )) * &
        (K_HS_NO3N_RED_INHB  / (NO3N   + K_HS_NO3N_RED_INHB )) * &
        (K_HS_MN_IV_RED_INHB / (MN_IV  + K_HS_MN_IV_RED_INHB))

    LIM_S_PLUS_6_RED = &
        (S_PLUS_6             / (S_PLUS_6 + K_HS_S_PLUS_6_RED_LIM)) * &
        (K_HS_DOXY_RED_INHB   / (DOXY     + K_HS_DOXY_RED_INHB   )) * &
        (K_HS_NO3N_RED_INHB   / (NO3N     + K_HS_NO3N_RED_INHB   )) * &
        (K_HS_MN_IV_RED_INHB  / (MN_IV    + K_HS_MN_IV_RED_INHB  )) * &
    ...
    where (REDUCED_AGENT_NO == 5)
        HS_MOLAR = &
            (S_MINUS_2 / S_MOLAR_MASS_MG) * &
            ((H_PLUS * 8.9D-8) / ((H_PLUS*H_PLUS) + (H_PLUS*8.9D-8) + (8.9D-8 * 1.2D-13)))

        PE = &
            4.25D0 - &
            log10((HS_MOLAR**0.125D0)/(((S_PLUS_6/S_MOLAR_MASS_MG) **0.125D0)*(H_PLUS**1.125D0)))
    end where
    ...
    ! FE_S_2 / DISS_FE_II
    FE_S_2_OVER_FE_II  = &
        10.0D0**(86.8D0 -(16.0D0*PH)+(2.0D0*log10((S_PLUS_6 / S_MOLAR_MASS_MG))) - (14.0D0*PE))
