...
subroutine READ_PELAGIC_MODEL_OPTIONS(IN_FILE)
    ...
    ! Which guilds the positional ratchet applies to (0 = all, default and
    ! byte-identical; 1 = NOST only; 2 = CYN only; 3 = the fixers only).
    read(IN_FILE + 1, *, end = 900, err = 900)
...
900 continue
    ...
    if (CYANO_POS_GUILDS /= 0) then
        write(*,*) 'Positional ratchet restricted to guild set ', CYANO_POS_GUILDS, &
                   ' (0=all, 1=NOST only, 2=CYN only, 3=fixers only)'
    end if
